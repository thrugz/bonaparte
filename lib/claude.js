...
const SYSTEM_BASE = `You are Bonaparte, a sharp strategic AI for a BIM data platform called Vitus. Your tone is direct, warm but no-fluff. Never use em dashes, bullet-point email bodies, or corporate language. Every recommendation names who does what by when. Lead with the answer.`;
...
function rolesBlock() {
  ...
  const lines = roles
    ...
    .map((r) => {
      ...
      const ids = [];
      if (r.hubspot_owner_id) ids.push(`HubSpot ownerId ${r.hubspot_owner_id}`);
      if (r.slack_user_id) ids.push(`Slack ${r.slack_user_id}`);
      if (ids.length) bits.push(`(${ids.join(", ")})`);
      return "- " + bits.join(" — ");
    });
  return `\n\nTeam at Vitus (use these names and titles when assigning actions; "the user" in interactive sessions is the first person listed who maps to the current session):\n${lines.join("\n")}`;
}

function buildSystem() {
  return SYSTEM_BASE + rolesBlock();
}
...
- Each action names who does what using the Team at Vitus roster in the system prompt — use first names and their stated title
...
- For BLUE: frame as opportunity ("<Account> has many contacts but no deal, worth creating one?")
...
/**
 * Extract durable business facts from a single chat turn.
 * Returns [{ type, description, tags }] — possibly empty.
 * Low temperature implicit via Haiku. Cheap; runs every turn.
 */
export async function extractFacts(userMessage, assistantMessage) {
  const prompt = `You extract durable business facts from a single Bonaparte chat turn.

USER: ${String(userMessage || "").slice(0, 2000)}
ASSISTANT: ${String(assistantMessage || "").slice(0, 2000)}

Rules:
- Extract ONLY things the user stated or confirmed as durable truth about Vitus, accounts, contacts, decisions, or identity. Example: "CN3 is an internal customer", "Josephine now owns COWI", "Bram prefers no em dashes".
- Skip speculation, questions, greetings, opinions, task requests, and anything ephemeral (like a specific week's signal — those live in HubSpot).
- Skip things already clearly stated in the assistant's recap of prior context.
- Prefer IDENTITY for durable traits of people/accounts ("CN3 is internal"), FACT for structural truths, DECISION for explicit choices, CONTEXT for strategic angles.
- Empty array is fine and often correct.

Return ONLY a JSON array, no prose, no fences:
[{"type":"IDENTITY","description":"CN3 is an internal Vitus customer","tags":["CN3","internal"]}]`;

  try {
    const text = await ask(prompt, 500, { fast: true });
    const cleaned = text.trim().replace(/^```json?\n?/, "").replace(/\n?```$/, "");
    const arr = JSON.parse(cleaned);
    if (!Array.isArray(arr)) return [];
    return arr
      .filter((f) => f && typeof f.description === "string" && f.description.trim().length > 3)
      .map((f) => ({
        type: ["FACT", "IDENTITY", "DECISION", "CONTEXT", "SIGNAL", "PATTERN"].includes(f.type) ? f.type : "FACT",
        description: String(f.description).trim(),
        tags: Array.isArray(f.tags) ? f.tags.slice(0, 6).map(String) : [],
      }));
  } catch {
    return [];
  }
...
/**
 * Generate the rolling business-context bulletin from the pre-baked slots.
 * Spacebot's intraday-synthesis shape: deterministic retrieval, one LLM
 * pass for prose. Called by the 30-min heartbeat, never by chat.
 */
export async function synthesizeBulletin(slots) {
  const list = (arr) => arr.map((n) => `- ${n.description}`).join("\n") || "(none)";
  const prompt = `Rewrite Bonaparte's rolling business-context brief from the pre-queried slots below. 75-150 words, narrative paragraphs, no em dashes, no bullet lists, no corporate language.

Identities:
${list(slots.identities)}

Urgent signals:
${list(slots.signals3)}

Recent decisions:
${list(slots.decisions)}

Patterns:
${list(slots.patterns)}

Highest-importance active nodes:
${list(slots.highImportance)}

Focus on: what is currently true about accounts and people, what is decided, what needs attention, what is in progress. Be specific with names and dates. Lead with the single most important thing. End with one concrete thing to watch this week.`;
