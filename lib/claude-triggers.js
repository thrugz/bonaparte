...
export async function getTrigger(id) {
  const r = await api("GET", `/v1/code/triggers/${id}`);
  return r?.trigger;
}

export async function updateTrigger(id, body) {
  const r = await api("POST", `/v1/code/triggers/${id}`, body);
  return r?.trigger;
}

export async function runTrigger(id) {
  return api("POST", `/v1/code/triggers/${id}/run`, {});
}
